...
color select_color; color select_text_color;
// LAYOUT CONDITIONS
boolean white_page;
...
void man_prp_ptg_tns_setup() { // man/prp/ptg/tns button goes on the bottom-left corner 
  float button_height = top_offset;
  float button_width = right_offset;
  // String[] split_string_into_lines (String s, float line_size)
...
void generic_graph_keyPressed() {
  ...
  } else if (key == '+') {zoom += .03; key_ok = true;
  } else if (key == '-') {zoom -= .03; key_ok = true;
